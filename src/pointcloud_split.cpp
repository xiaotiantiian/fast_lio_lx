        ...
        PointCloudXYZI::Ptr &feats_combined)
    {
        ...
        if (!feats_undistort_prev->empty())
        {
            for (const auto &pt : feats_undistort_prev->points)
            {
                ...
                V3D p_lidar_curr = state_point.offset_R_L_I.conjugate() * (state_point.rot.conjugate() * (state_prev.rot * (state_prev.offset_R_L_I * p_lidar_prev + state_prev.offset_T_L_I) +
